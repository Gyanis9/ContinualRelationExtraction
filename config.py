...
def get_config():
    ...
    parser.add_argument('--task_name', default='TACRED', type=str,
                        help="任务名称，例如 TACRED 数据集")
    parser.add_argument('--device', default='cuda', type=str,
                        help="训练设备，支持 'cuda' 或 'cpu'")
    parser.add_argument('--memory_size', default=10, type=int,
                        help="每个任务的记忆大小，控制每个任务保存的信息量")

    # ================================== 数据集设置 ===================================
    parser.add_argument('--num_of_relations', default=40, type=int,
                        help="数据集中总的关系数量")
    parser.add_argument('--max_sequence_length', default=256, type=int,
                        help="输入序列的最大长度，超过该长度会被截断")
    parser.add_argument('--relations_per_task', default=4, type=int,
                        help="每个任务中包含的关系数量")
    parser.add_argument('--num_of_train_samples', default=420, type=int,
                        help="训练集样本数量")
    parser.add_argument('--num_of_val_samples', default=140, type=int,
                        help="验证集样本数量")
    parser.add_argument('--num_of_test_samples', default=140, type=int,
                        help="测试集样本数量")
    ...
    # =================================== 模型设置 ===================================
    parser.add_argument('--bert_model_path', default='bert-base-uncased', type=str,
                        help="BERT模型的预训练路径")
    parser.add_argument('--vocab_size', default=30522, type=int,
                        help="词汇表大小")
    parser.add_argument('--dropout_rate', default=0, type=float,
                        help="Dropout比率，用于防止过拟合")
    parser.add_argument('--encoding_mode', default='entity_marker', type=str,
                        help="实体标记方式，可选 'entity_marker' 或其他方式")
    ...
    parser.add_argument('--max_grad_norm', default=10, type=int,
                        help="最大梯度裁剪阈值，用于防止梯度爆炸")
    parser.add_argument('--additional_classifier', default=1, type=int,
                        help="是否使用额外的分类器，0表示不使用，1表示使用")
    parser.add_argument("--encoder_lr", default=1e-5, type=float,
                        help="编码器的初始学习率，用于AdamW优化器")
    parser.add_argument("--classifier_lr", default=1e-3, type=float,
                        help="分类器的初始学习率，用于AdamW优化器")

    parser.add_argument("--temperature", default=0.007, type=float)

    # ==================================== GradScaler训练参数设置 ================================
    parser.add_argument("--use_amp", default=False, type=bool)
    parser.add_argument("--init_scale", default=2. ** 16, type=float)
    parser.add_argument("--growth_interval", default=2000, type=int)
